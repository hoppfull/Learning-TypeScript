module Engine {
    ...
    export function bufferCubeMap(gl: WebGLRenderingContext, posY: HTMLImageElement, negY: HTMLImageElement, posX: HTMLImageElement, negX: HTMLImageElement, posZ: HTMLImageElement, negZ: HTMLImageElement) {
        const TBO = gl.createTexture()
    ...
    export function useCubeMap(gl: WebGLRenderingContext, {id, location}: { id: number, location: WebGLUniformLocation }, TBO: WebGLTexture) {
        gl.activeTexture(gl.TEXTURE0 + id)
        gl.bindTexture(gl.TEXTURE_CUBE_MAP, TBO)
        gl.uniform1i(location, id)
    }
